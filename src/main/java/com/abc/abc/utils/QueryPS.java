...
public String getKryById = """
        CREATE OR REPLACE FUNCTION public.getkaryawanbyid(rqid bigint)
         RETURNS TABLE(resid bigint, resnama character varying, resjk character varying, resdob date, resalamat text, resstatus character varying, rescreated_date date, resupdated_date date, resdeleted_date date)
         LANGUAGE plpgsql
        AS $function$
        	BEGIN
          RETURN QUERY
            select k.id, k.nama, k.jk, k.dob, k.alamat, k.status, k.created_date, k.updated_date, k.deleted_date
